...
function ActionModal({ isOpen, type, targetItem, onClose, onConfirm }) {
  ...
  const actualName = targetItem?.agentDetails?.name || "";
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-200 flex items-center justify-center p-6">
      <div className="bg-white border-[3px] border-black p-8 rounded-[40px] w-full max-w-md shadow-[15px_15px_0px_0px_rgba(0,0,0,1)]">
        ...
        <p className="text-gray-500 text-sm mb-6">
          To {type}{" "}
          <span className="text-black font-black italic">"{actualName}"</span>,
          type their name below.
        </p>

        <input
          className="w-full bg-gray-50 border-2 border-black p-4 rounded-2xl font-bold mb-6 outline-none focus:bg-white transition-colors"
        ...
        <div className="flex gap-4">
          <button
            onClick={onClose}
            className="flex-1 p-4 border-2 border-black rounded-2xl font-black uppercase text-xs hover:bg-zinc-100 transition-all"
          >
            Cancel
          </button>
          <button
